...
def count_accuracy(true,false,id,Id):
    ...
    return true,false
# 利用单步修正的梯度下降算法设计的感知器
def processing_step(A1,A2,ID,del_line,w,p):
    '''
    (1)去除无用的数据
    ...
    i = 0 # 目前在用第i个数据检验正确性
    print("采用单步处理的梯度下降算法迭代计算第%d类和%d类的权向量:"%(ID[0],ID[1]))
    # '''开始迭代！！'''
    while 1:
        y = np.dot(A[i,:],w)
        if y <= 0:
...
def Plot(A1,A2,w,ID,flag):
    ...
    ax.plot_surface(X,Y,Z,color='aliceblue')  # type: ignore
...
p = 0.95 # 学习率
# 求解第一类和第二类的权向量w 
w1 = processing_step(A1,A2,[1,2],del_line,w,p)
# 求解第一类和第三类的权向量w 
w2 = processing_step(A1,A3,[1,3],del_line,w,p)
# 求解第二类和第三类的权向量w 
w = 0.01*np.ones((4,1)) # 初始权向量
p = 0.001 # 学习率
b = 0.001*np.mat(np.ones((50,1)))
n_interations = int(1e5) # 迭代次数
w3 = LMS(A2,A3,[2,3],del_line,w,b,p,n_interations)


'''
4.绘制训练集的散点图以及分类面
flag为1则为训练集;flag为0则为测试集
'''
...
# 第二类和第三类的分类面以及训练集散点图
Plot(a2,a3,w3,[2,3],flag=1)
plt.title('第二类和第三类训练集的分布')
'''
5.导入测试集对分类面进行检验
'''
# 检验第一类和第二类的准确率
res12,res21 = Test(B1,B2,del_line,[1,2],w1,2)
# 检验第一类和第三类的准确率
res13,res31 = Test(B1,B3,del_line,[1,3],w2,2)
# 检验第二类和第三类的准确率
res23,res32 = Test(B2,B3,del_line,[2,3],w3,3)
'''
6.绘制测试集的散点图以及分类面
flag为1则为训练集;flag为0则为测试集
'''
# 第一类和第二类的分类面以及测试集集散点图
Plot(res12,res21,w1,[1,2],flag=2)
plt.title('第一类和第二类测试集的分布')
# 第一类和第三类的分类面以及测试集散点图
Plot(res13,res31,w2,[1,3],flag=2)
plt.title('第一类和第三类测试集的分布')
# 第二类和第三类的分类面以及测试集散点图
Plot(res23,res32,w3,[2,3],flag=0)
plt.title('第二类和第三类测试集的分布')
